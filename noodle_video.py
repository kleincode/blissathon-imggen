...
ZODIAC_PROMPT = """
Generate an Instagram post for me where each zodiac sign is assigned to a kind of pasta.
...
while True:
    end_files = gen_img("Berlin full of pizzas", 1)
    if end_files:
        img = Image.open(end_files[0])
        draw = ImageDraw.Draw(img)
        txt = f"Your favorite kind of pasta\nbased on your zodiac sign!!!"
...
while True:
    end_files = gen_img("A pizza standing in victory, the pasta looks defeated", 1)
    if end_files:
        img = Image.open(end_files[0])
        draw = ImageDraw.Draw(img)
        txt = f"PS: Pizza master race!!!!"
        draw.multiline_text((1536//2, 1536//2), txt, fill=(255, 255, 255), stroke_fill=(0, 0, 0), stroke_width=2, anchor="mm", font_size=FONT_SIZE, align="center")
