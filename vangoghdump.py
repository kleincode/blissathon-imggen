...
# Picasso sometimes has difficulties generating images, same with Michelangelo
...
artist = "Neanderthal"
# generate 4 images
for i, prompt in enumerate([f"Cute moment in a cafe with friends in the style of {artist}", f"Working on a computer in the style of {artist}", f"Sunrise in a park in Berlin in the style of {artist}", f"Partying in Berlin with friends the style of {artist}"]):
...
new_image.paste(image_1,(image_size[0],0))
new_image.paste(image_2,(image_size[0], image_size[1]))
new_image.paste(image_3,(0, image_size[0]))
...
new_image.save(f"vangogh/dump_image_{artist}.png","PNG")
new_image.show()
